...
import com.photocomposer.subjectdetection.util.resize2
import com.photocomposer.subjectdetection.util.translate2

/**
 * Wrapper class for Classifier Interface
 */
class SubjectDetectorLocalizerAPI : Detector {
    companion object {
        ...
        private const val DEFAULT_SUBJECT_DETECTION_LABELS = "file:///android_asset/subject_localizer_labels.txt"
        private const val DEFAULT_NUM_THREADS = 4

        private const val DEFAULT_NUM_BYTES_PER_CHANNEL = 1
        private const val DEFAULT_MAX_NUM_DETECTIONS = 100
        ...
        fun createDefault(assetManager: AssetManager): Detector {
            ...
            val subjectDetector = SubjectDetectorLocalizerAPI()
            // Open subject detection label
            val labelFileName = DEFAULT_SUBJECT_DETECTION_LABELS.split("file:///android_asset/".toRegex()).toTypedArray()[1]
    ...
    override fun detectSubjects(img: Bitmap): List<SubjectDetected> {
        val bitmapImage = img.resize2()
        // Preprocess the image data form 0-255 int

        bitmapImage.getPixels(intValues,
            0,
            bitmapImage.width,
            0,
            0,
        ...
        for (i in 0 until DEFAULT_MAX_NUM_DETECTIONS) {
            ...
            img.translate2(subjectBoundingBox)
            val labelOffset = 1
            ...
            if (confidenceScore >= DEFAULT_MIN_CONFIDENCE) {
                subjectsDetected.add(SubjectDetected(id = i,
